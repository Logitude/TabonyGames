...
def create_match(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            request.session['title'] = form_data.get('title')
            request.session['player_count'] = form_data.get('player_count')
            request.session['growth_resources'] = form_data.get('growth_resources')
            request.session['player1'] = form_data.get('player1')
            request.session['player2'] = form_data.get('player2')
            request.session['player3'] = form_data.get('player3')
            request.session['player4'] = form_data.get('player4')
            request.session['player5'] = form_data.get('player5')
            request.session['player6'] = form_data.get('player6')
            request.session['extra_draft_nations'] = form_data.get('extra_draft_nations')
            request.session['resource_remainder_tiebreaker'] = form_data.get('resource_remainder_tiebreaker')
            request.session['weighted_card_draw'] = form_data.get('weighted_card_draw')
            request.session['korea_nerf'] = form_data.get('korea_nerf')
            request.session['lincoln_nerf'] = form_data.get('lincoln_nerf')
...
def confirm_create_match(request):
    data = {}
    if request.method == 'POST':
        getter = request.session.pop
    else:
        getter = request.session.get
    data['title'] = getter('title', None)
    data['player_count'] = getter('player_count', None)
    data['growth_resources'] = getter('growth_resources', None)
    data['player1'] = getter('player1', None)
    data['player2'] = getter('player2', None)
    data['player3'] = getter('player3', None)
    data['player4'] = getter('player4', None)
    data['player5'] = getter('player5', None)
    data['player6'] = getter('player6', None)
    data['extra_draft_nations'] = getter('extra_draft_nations', None)
    data['resource_remainder_tiebreaker'] = getter('resource_remainder_tiebreaker', None)
    data['weighted_card_draw'] = getter('weighted_card_draw', None)
    data['korea_nerf'] = getter('korea_nerf', None)
    data['lincoln_nerf'] = getter('lincoln_nerf', None)
    ...
    if request.method == 'POST':
        ...
        match = Match.objects.create(title=data['title'], player_count=data['player_count'], growth_resources=data['growth_resources'])
        match.extra_draft_nations = data['extra_draft_nations']
        if data['resource_remainder_tiebreaker']:
            match.resource_remainder_tiebreaker = True
        if data['weighted_card_draw']:
            match.weighted_card_draw = True
        if data['korea_nerf']:
            match.korea_nerf = True
        if data['lincoln_nerf']:
    ...
    return render(request, 'Nations/confirm_create.html', {'turns': number_of_turns(request.user), 'data': data})
